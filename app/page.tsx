...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-white text-slate-950">
      ...
      <section className="border-y bg-slate-50"><div className="mx-auto max-w-7xl px-6 py-16 lg:px-8"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="text-sm font-bold uppercase tracking-widest text-blue-600">Featured</p><h2 className="mt-2 text-3xl font-bold">Our 5 Picks</h2><p className="mt-3 text-slate-600">Our recommendations, your choice. Start with these five student-focused picks, then compare the full shortlist.</p></div><Link href="/laptops/best-students" className="font-bold text-blue-600">Compare all {laptops.length} →</Link></div><div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-5">{top.map(p=><div key={p.asin} className="rounded-2xl border border-slate-200 bg-white p-5"><div className="mb-4 flex h-28 items-center justify-center rounded-xl bg-slate-50 p-2"><img src={p.imageUrl} alt="" className="h-full w-full object-contain" loading="lazy" /></div><p className="text-xs font-bold text-blue-600">OUR PICK · MBA {p.score}/10</p><h3 className="mt-2 font-bold">{p.brand} {p.model}</h3><p className="mt-2 text-sm text-slate-500">{p.price}</p></div>)}</div></div></section>
